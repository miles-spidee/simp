...
export default function PlacementPipeline() {
  ...
  return (
    <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-border shadow-sm overflow-hidden mt-6">
      <div className="p-4 border-b border-border flex justify-between items-center bg-white/50">
        <h2 className="text-lg font-bold text-text-primary tracking-tight flex items-center gap-2">
          <Briefcase className="h-5 w-5 text-text-secondary" /> Active Hiring Pipeline
        </h2>
      </div>
                ...
                <tr key={pl.id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-medium text-text-primary">{pl.studentName}</span>
                      <span className="text-xs text-text-secondary">{pl.program}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="font-semibold text-text-primary">{pl.companyName}</span>
                      <span className="text-xs text-text-secondary mt-0.5">{pl.role} • {pl.location}</span>
                    </div>
                  </td>
