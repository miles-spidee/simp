...
export function ReviewApplicationDrawer({ isOpen, onClose, application, onApplicationUpdated }: ReviewApplicationDrawerProps) {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'WITHDRAWN':
        return 'bg-slate-100 text-text-primary border-border';
      case 'DRAFT':
      ...
      <div className="flex flex-col h-full min-h-0 bg-slate-50">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {activeTab === 'profile' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-white border border-border rounded-xl p-5 shadow-sm space-y-4">
                <h3 className="text-sm font-semibold text-text-primary border-b border-border pb-2">Candidate Details</h3>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-lg font-bold text-text-primary">{application.candidateName}</div>
                    <div className="flex items-center gap-4 mt-1 text-sm text-text-secondary">
                      <span className="flex items-center gap-1.5"><Mail className="h-4 w-4 text-text-secondary" /> {application.email}</span>
                    </div>
                    <div className="flex items-center gap-4 mt-1 text-sm text-text-secondary">
                      <span className="flex items-center gap-1.5"><Phone className="h-4 w-4 text-text-secondary" /> {application.phone}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {['Resume', 'ID Proof', 'Certificates'].map((doc, idx) => (
                <div key={idx} className="bg-white p-4 rounded-xl border border-border flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-semibold text-sm text-text-primary">{doc}</div>
                      <div className="text-xs text-text-secondary">PDF • Uploaded {application.appliedDate}</div>
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button className="p-2 text-text-secondary hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
          ...
          {activeTab === 'review' && (
            <div className="space-y-6 animate-in fade-in duration-300">
              <div className="bg-white border border-border rounded-xl p-5 shadow-sm space-y-4">
                <h3 className="text-sm font-semibold text-text-primary border-b border-border pb-2">Application Info</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <p className="text-xs text-text-secondary font-bold uppercase tracking-wider">Applied For</p>
                    <div className="flex items-center gap-1.5 mt-1 text-sm font-semibold text-text-primary bg-slate-50 p-3 rounded-lg border border-border">
                      <Briefcase className="h-4 w-4 text-blue-500" />
                      {application.opportunityData?.title || application.opportunityId}
                    </div>
                  </div>
                  <div>
                    <p className="text-xs text-text-secondary font-bold uppercase tracking-wider">Date Applied</p>
                  ...
                  <div>
                    <p className="text-xs text-text-secondary font-bold uppercase tracking-wider">Current Status</p>
                    <div className="mt-1">
        ...
        {/* Action Footer */}
        {activeTab === 'review' && (
          <div className="shrink-0 border-t border-border p-6 bg-white flex flex-col gap-4 animate-in slide-in-from-bottom-2">
            <p className="text-xs font-bold text-text-secondary uppercase tracking-widest text-center">Update Application Status</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <button 
                ...
                className="flex flex-col items-center justify-center gap-1.5 p-3 rounded-xl border-2 border-blue-200 bg-blue-50 text-blue-700 hover:bg-blue-100 hover:border-secondary disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
