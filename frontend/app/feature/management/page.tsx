    ...
    <div className="space-y-6 animate-slide-in select-none">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-black text-text-primary mt-2 tracking-tight">Assignment Management</h2>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left p-4 hover:bg-slate-50 transition-colors flex items-center justify-between cursor-pointer ${
                    isActive ? 'bg-blue-50/40 text-blue-600 font-bold border-l-4 border-blue-600' : 'text-text-primary'
                  }`}
                >
                  <div className="min-w-0 pr-2">
                    ...
                    <span className="text-[10px] text-text-secondary font-semibold mt-1 block">Due: {task.dueDate}</span>
                  </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          {selectedTask && (
            <div className="bg-white border border-border rounded-2xl p-6 shadow-sm space-y-6">
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 pb-5 border-b border-border">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="text-text-secondary">• Created: {selectedTask.assignedDate}</span>
                  </div>
                  <h3 className="text-lg font-bold text-text-primary mt-2">{selectedTask.title}</h3>
                  <p className="text-xs text-text-secondary leading-relaxed">{selectedTask.description}</p>
                </div>

                <div className="flex flex-col items-end gap-1 text-right shrink-0">
                  <span className="text-[9px] font-bold text-text-secondary uppercase tracking-widest">Deadline</span>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {taskSubmissions.map((sub) => {
                    ...
                    return (
                      <div 
                        ...
                        className={`p-4 border rounded-xl flex flex-col md:flex-row md:items-center justify-between gap-4 cursor-pointer hover:border-secondary transition-all ${
                          isSelected 
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-0.5 text-[9px] text-text-secondary font-bold">
                  ...
                  {taskSubmissions.length === 0 && (
                    <div className="p-8 text-center text-text-secondary italic bg-slate-50 border border-border rounded-xl text-xs">
              ...
              {/* EVALUATION BLOCK */}
              {selectedSubmission && (
                <div className="border-t border-border pt-5 mt-5 space-y-4 animate-slide-in">
                  <div className="flex items-center justify-between">
                    <h4 className="font-bold text-xs text-text-primary uppercase tracking-widest flex items-center gap-2">
                      <User className="h-4.5 w-4.5 text-blue-600" />
                      Evaluating: {selectedSubmission.studentName}
                    </h4>
                    <button 
                      onClick={() => setSelectedSubId(null)}
                      className="text-text-secondary hover:text-text-secondary cursor-pointer"
                    >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <a 
                      ...
                      target="_blank" 
                      rel="noreferrer"
                      className="flex items-center justify-between p-3 border border-border rounded-xl hover:border-secondary transition-colors text-xs font-semibold text-text-primary bg-white"
                    >
                      <span className="truncate">GitHub: {selectedSubmission.githubUrl}</span>
                      <ExternalLink className="h-3.5 w-3.5 text-text-secondary shrink-0 ml-2" />
                    </a>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-text-secondary uppercase block">Submission Feedback</label>
                    <textarea 
                      rows={3} 
      ...
      {showTaskForm && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleCreateTask} className="bg-white border border-border rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-4">
            <h3 className="font-bold text-sm text-text-primary uppercase tracking-wider border-b border-border pb-2">Publish New Assignment</h3>
            
            <div className="space-y-1">
              <label className="text-[9px] font-bold text-text-secondary uppercase block">Assignment Title</label>
              <input 
                type="text" 
                required 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-text-secondary uppercase block">Deadline Date</label>
                <input 
                  type="date" 
                  required 
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-bold text-text-secondary uppercase block">Max Attempts</label>
                <input 
                  type="number" 
                  min={1} 
                  required 
            ...
            <div className="space-y-1">
              <label className="text-[9px] font-bold text-text-secondary uppercase block">Description / Requirements</label>
              <textarea 
                required
                rows={3} 
