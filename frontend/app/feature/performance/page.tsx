  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
      <div className="bg-white border-b border-border px-6 py-4 flex items-center justify-between shrink-0">
        <div>
          <h1 className="text-xl font-bold text-text-primary">Performance Module</h1>
          <p className="text-sm text-text-secondary mt-1">Track and analyze student and batch performance.</p>
        </div>
          ...
          <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white border border-border rounded-2xl p-5 shadow-sm space-y-4">
                <h3 className="text-xs font-bold text-text-secondary uppercase tracking-widest flex items-center gap-1.5 border-b border-border pb-2">
                  ...
                  {performances.filter(p => p.average_score >= 90).map((perf) => (
                    <div key={perf.studentId} className="p-3 bg-slate-50 border border-border rounded-xl flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-helper mt-1">Batch: {perf.batchId}</p>
                      </div>
            ...
            <div className="bg-white border-b border-border px-6 py-4 shrink-0">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-text-primary">Student: {selectedPerformance.studentId}</h2>
                    <p className="text-sm text-text-secondary">Batch: {selectedPerformance.batchId}</p>
                  </div>
                </div>
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  <div className="bg-white p-5 rounded-xl border border-border shadow-sm space-y-4">
                    <h3 className="text-sm font-semibold text-text-primary border-b border-border pb-2">Core Metrics</h3>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <span className="block text-xs font-bold text-text-secondary uppercase tracking-wider mb-1">Overall Average</span>
                        <span className="font-medium text-text-primary">{selectedPerformance.average_score}%</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-text-secondary uppercase tracking-wider mb-1">Attendance Rate</span>
                        <span className="font-medium text-text-primary">{selectedPerformance.attendance_rate}%</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-text-secondary uppercase tracking-wider mb-1">Task Completion</span>
                        <span className="font-medium text-text-primary">{selectedPerformance.task_completion_rate}%</span>
                      </div>
                      <div>
                        <span className="block text-xs font-bold text-text-secondary uppercase tracking-wider mb-1">Assessment Avg</span>
                        <span className="font-medium text-text-primary">{selectedPerformance.assessment_score}%</span>
                      </div>
                    </div>
                  </div>
                </div>
