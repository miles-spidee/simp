          ...
          <div className="space-y-6 animate-slide-in">
            ...
            <div className="bg-slate-50 border border-slate-200 p-6 rounded-2xl space-y-4">
              <div className="flex flex-col md:flex-row items-center justify-around gap-6 text-center">
                {[
                  ...
                  { name: 'Attendance Record', type: 'Cascade Cascade' },
                  { name: 'Assessments Log', type: 'Cascade Restrict' }
                ].map((entity, idx) => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs font-semibold">
              ...
              <div className="border border-slate-200 p-4 rounded-xl border-amber-250 bg-amber-50/20">
                <span className="block text-slate-400 font-bold text-[10px] uppercase mb-1 text-amber-700">Broken Foreign Keys</span>
                <span className="text-amber-700 font-black text-sm">1 Warning</span>
                <p className="text-[11px] text-amber-600 mt-1 font-medium">LMS tracking references contains 4 logs pointing to deleted mock video files.</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={card.title} className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                  <span className="text-[9px] text-slate-400 font-extrabold uppercase tracking-widest">{card.title}</span>
                ...
                <tbody className="divide-y divide-slate-100 font-medium">
                  {modules.map((m) => (
                    <tr key={m.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3">
                        <span className={`font-bold ${m.hasExcelExport ? 'text-emerald-600' : 'text-slate-400'}`}>
                ...
                <tbody className="divide-y divide-slate-100 font-medium">
                  {filteredTestCases.map((tc) => (
                    <tr key={tc.id} className="hover:bg-slate-50/50">
                      ...
                      <td className="p-3 text-slate-600 font-semibold leading-normal max-w-sm">{tc.scenario}</td>
